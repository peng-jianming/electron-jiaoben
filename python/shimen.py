...
def create_shimen_task(device_id):
    ...
    @sm.state(配置.活动界面)
    def _(context):
        if not sm.Field(配置.活动界面['按钮']['师门任务']).设置标识('点击参加').查找().点击().随机延时(1, 3).是否找到():
            sm.stop()
    ...
    def _(context):
            if sm.Field(配置.主界面['按钮']['使用']).设置标识('点击使用按钮').查找().点击().随机延时(1, 3).是否找到():
                return
            if sm.Field(配置.主界面['按钮']['对话框第一个选项按钮']).设置标识('点击对话框第一个选项按钮').查找().点击().随机延时(1, 3).是否找到():
                return
            if sm.Field(配置.主界面['状态']['是否未选中任务栏']).设置标识('选中任务栏').查找().点击().随机延时(1, 3).是否找到():
                return
            if sm.Field(配置.主界面_师门文字).设置标识('点击任务面板的师门文字').查找().点击().随机延时(1, 3).是否找到():
                return
