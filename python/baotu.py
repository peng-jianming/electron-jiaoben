...
def create_baotu_task(device_id):
    ...
    @sm.state(配置.活动界面)
    def _(context):
        if not sm.Field(配置.活动界面['按钮']['宝图任务']).设置标识('点击参加').查找().点击().随机延时(1, 3).是否找到():
            sm.stop()

    @sm.state(配置.战斗界面)
    def _(context):
        sm.Field(配置.战斗界面['状态']['是否未准备战斗']).设置标识('点击未准备战斗按钮').查找().点击().随机延时(1, 3)
    ...
    def _(context):
        ...
        if sm.Field(配置.主界面_宝图文字).设置标识('点击任务面板的宝图任务文字').查找().点击().随机延时(1, 3).是否找到():
            return
