...
class 状态机环境:
    ...
    账号信息: dict
    字库缓存: dict | None = None
    图片库缓存: dict | None = None
    检测模型: object | None = None

    def 创建图色工具包(self, 日志前缀: str = ""):
        """任务内自由组合识别/点击，与 JSON 元素配置无关。详见 core.图色工具。"""
        from .图色工具 import 图色工具包类
...
class 任务界面状态机类:
    ...
    def __init__(self, 环境):
        ...
        self._当前任务所有界面公共处理器 = []
        self.任务未完成 = True
        # 与 状态机环境 对齐的统一上下文字典；动态键仍通过 更新上下文 写入
        self.上下文 = {
            "控制器": 环境.控制器,
            "截图上下文": 环境.截图上下文,
            "界面集合": 环境.界面集合,
        ...
        self._任务超时时间 = None
        self._轮流键 = 环境.账号信息.get("id") if 启用轮流单执行 else None
    ...
    def 更新上下文(self, **kwargs):
        """合并任务级动态状态到 self.上下文（勿覆盖 控制器/环境 等固定键名）。"""
