...
class 任务管理器类:
    ...
    def 运行(self):
        所有任务映射 = self.获取所有任务列表()
        总数 = len(self.任务配置列表)
        轮流键 = self.账号信息.get("id")
        try:
            if 启用轮流单执行:
                全局轮流门闩.注册(轮流键)
    ...
    def 加载界面配置(self, 界面配置文件路径):
        ...
        for 界面名称, 原始配置 in 界面集合.items():
            # 创建界面配置对象（内部含整屏识别 动作管理器，见 界面管理器类）
            配置[界面名称] = 界面管理器类(
                界面名称,
                原始配置,
                self.控制器,
...
class 任务界面状态机类:
    ...
    def __init__(self, 任务管理器, 超时时间 = None):
        self.任务管理器 = 任务管理器
        self.注册界面集合 = {}
        self.任务未完成 = True
        self.上下文 = {}
    ...
    def _轮流等待(self):
        全局轮流门闩.等待轮到(self._轮流键)

    def _轮流让出(self):
        全局轮流门闩.结束本轮让出(self._轮流键)
    ...
    def _尝试执行误触(self, 区域=None):
        ...
        self.任务管理器.更新数据("日志", f"[误触模拟] 误触 - 类型: {误触类型}")

        if 误触类型 == "点击":
            self.任务管理器.控制器.随机点击(区域)
        elif 误触类型 == "滑动":
            pass
    ...
    def 保存未知图片(self, 图像数据):
        ...
        try:
            ...
            self.任务管理器.更新数据("日志", f"未知界面截图已保存: {目标路径}")
        except Exception:
            self.任务管理器.更新数据("日志", "保存未知界面截图失败")
        ...
        try:
            if os.path.exists(音乐文件路径):
                try:
                    ...
                    self.任务管理器.更新数据("日志", "提示音乐播放结束")
                except Exception:
                    self.任务管理器.更新数据("日志", "播放音乐失败")
            else:
                self.任务管理器.更新数据("日志", f"音乐文件不存在: {音乐文件路径}")
        except Exception as e:
            self.任务管理器.更新数据("日志", "播放音乐出错")
        ...
        while self.任务未完成:
            ...
            try:
                ...
                for 界面名称 in (
                    ...
                    if self.任务管理器.界面集合[界面名称].查找().是否找到():
                        已找到 = True
                        if self._尝试执行误触(self.任务管理器.界面集合[界面名称].误触区域):
                            continue
                        self.上下文["上次识别界面名"] = 当前界面名
                        当前界面名 = 界面名称
                        if 未知开始时间 is not None:
                            self.任务管理器.更新数据("故障", False)
                        未知开始时间 = None
                        self._执行界面处理链(界面名称)
                        break
                ...
                if not 已找到:
                    ...
                    # 尝试关闭未注册界面
                    for 界面名称, 界面对象 in self.任务管理器.界面集合.items():
                        if 界面名称 in self.注册界面集合:
                            continue
                        if hasattr(界面对象.元素, "关闭"):
                            if self.任务管理器.界面集合[界面名称].查找().是否找到():
                                是否处于未知界面 = False
                                界面对象.元素.关闭.点击()
                                self.任务管理器.更新数据(
