...
class DeviceController:
    ...
    def opencv颜色偏色找图2(self, large_image_path, small_image_path, color_tolerance, similarity=0.8, region=(0, 0, 0, 0)):
        """
        修复版本：基于完整大图进行颜色偏色处理，然后裁剪检测区域
        """
        # 读取图像
        ...
        # 获取完整大图尺寸
        full_h, full_w = large_array.shape[:2]
        ...
        # 计算颜色上下限
        lower_bound = np.clip(base_color - tolerance, 0, 255)
        upper_bound = np.clip(base_color + tolerance, 0, 255)
        
        # 使用颜色范围创建掩码
        search_mask = cv2.inRange(large_array, lower_bound, upper_bound)
        small_mask = cv2.inRange(small_array, lower_bound, upper_bound)
        
        # 反转掩码（因为 inRange 返回的是符合条件的为255）
        full_binary = search_mask
        small_binary = small_mask
        
        # # 关键修改：先对整个完整大图进行颜色偏色处理
        # large_int16 = large_array.astype(np.int16)
        # large_diff = np.abs(large_int16 - base_color)
        # large_mask = np.all(large_diff <= tolerance, axis=2)
        # full_binary = np.where(large_mask, 255, 0).astype(np.uint8)
        ...
        # 显示图像
        cv2.imshow("large_array", full_binary)
        cv2.imshow("small_array", small_binary)
        ...
        # 判断是否指定了检测区域
        if x == 0 and y == 0 and width == 0 and height == 0:
            search_binary = full_binary
            offset_x, offset_y = 0, 0
            search_h, search_w = full_h, full_w
        else:
            # 确保区域在图像范围内
            x = max(0, x)
            y = max(0, y)
            ...
            crop_x1 = x
            crop_y1 = y
            crop_x2 = min(x + width, full_w)
            crop_y2 = min(y + height, full_h)
            ...
            # 从完整二值化图像中裁剪检测区域
            search_binary = full_binary[crop_y1:crop_y2, crop_x1:crop_x2]
            offset_x, offset_y = crop_x1, crop_y1
            search_h, search_w = search_binary.shape
    ...
    def opencv找透明图(self, large_img_path, small_img_path, tolerance=0, similarity=0.9, region=(0, 0, 0, 0)):
        ...
        try:
            ...
            # cv2.imshow('small_bgr', small_bgr)
