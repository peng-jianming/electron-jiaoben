...
class 主程序:
    def __init__(self):
        ...
        # 线程控制器：负责账号任务线程,
        # 后续如果任务代码是在设备上,那么这个线程控制器可以换成功能差不多的与设备通讯管理的类。
        self.线程控制器 = 线程控制器类(
            回调函数=任务管理器类,
    ...
    def 客户端消息队列处理(self, 类型, 数据):
        ...
        处理器 = {
            ...
            "设置账号任务配置列表": lambda d: self._设置账号任务配置列表(d.get("账号"), d.get("任务配置列表")),
    ...
    def _设置账号任务配置列表(self, 账号, 任务配置列表):
        """设置账号任务配置并推送到前端，保证刷新后点击开始时使用最新配置。"""
        if not 账号 or 任务配置列表 is None:
            return
        self.账号管理器.设置任务配置列表(账号, 任务配置列表)
        self.更新账号数据(账号, "任务配置列表", 任务配置列表)
