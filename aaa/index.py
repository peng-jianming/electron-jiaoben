...
def create_overlay(img1, img2, dx, dy):
    """
    生成红绿叠加图：img1 红色，img2 绿色，重叠区域为黄色。
    返回叠加图（BGR格式）和画布尺寸。
    """
    h1, w1 = img1.shape
    h2, w2 = img2.shape
    ...
    x1 = -x_min
    y1 = -y_min
    roi1 = overlay[y1:y1+h1, x1:x1+w1, 2]  # 红色通道
    roi1[img1 == 1] = 255

    # 放置 img2（绿色通道）
    x2 = -x_min + dx
    y2 = -y_min + dy
    roi2 = overlay[y2:y2+h2, x2:x2+w2, 1]  # 绿色通道
    roi2[img2 == 1] = 255
...
if __name__ == "__main__":
    ...
    # 3. 生成叠加图，让用户检查
    overlay = create_overlay(b1, b2, dx, dy)
    cv2.imwrite(overlay_path, overlay)
    print(f"叠加检查图已生成: {overlay_path}")
    print("请在图片查看器中打开该文件，检查红色和绿色是否对齐。")
    print("对齐良好的标志：重叠区域呈现黄色，且边界连续。")
