...
def _(context):
        # 点击抓鬼任务 参加按钮
       if not Field(zhuagui_assets.活动弹框_抓鬼任务).查找().偏移点击(308, 33,116, 56).随机延时(1, 2).是否找到():
           sm.stop()
...
def _(context):
        ...
        if Field(common_assets.队伍没满).查找().是否找到():
            # 一键喊话
            ...
            #关掉
            Field(common_assets.队伍弹框界面).查找().关闭().随机延时(2, 3)
...
def _(context):
    Field(common_assets.继续抓鬼提示).查找().关闭().随机延时(1, 2)
...
@sm.state(zhuagui_assets.队伍缺辅助)
def _(context):
    Field(zhuagui_assets.队伍缺辅助).查找().关闭().随机延时(1, 2)
...
def _(context):
        Field(zhuagui_assets.钟馗对话).查找().点击(1829, 361, 403, 66).随机延时(2, 3)

        Field(common_assets.主界面_未选中任务).查找().点击().随机延时(1,2)

        if Field(zhuagui_assets.主界面_抓鬼文字).查找().偏移点击(0, 0, 350, 114).随机延时(1, 2).是否找到():
            return
