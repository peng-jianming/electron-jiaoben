...
def opencv找透明图2222(large_img_path, small_img_path, similarity=0.9, region=(0, 0, 0, 0)):
    """
    ...
    try:
        ...
        # print( "2222222", final_similarity)
        print('5555', best_val, start_x, start_y)
        # 11. 返回结果
        return {
            "x": start_x,
            "y": start_y,
...
#     # 将 BGRA 转换为 BGR（3通道）用于模板匹配
#     search_area_bgr = cv2.cvtColor(search_area, cv2.COLOR_BGRA2BGR)
#     small_image_bgr = cv2.cvtColor(small_image, cv2.COLOR_BGRA2BGR)

#     # 使用模板匹配
#     result = cv2.matchTemplate(search_area_bgr, small_image_bgr, cv2.TM_CCOEFF_NORMED, mask=mask)
