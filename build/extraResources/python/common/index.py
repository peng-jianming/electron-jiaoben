    ...
    def __init__(self):
        ...
        def 识别屏幕():
            ...
            if Field(common_assets.活动界面).设置大图路径(url).查找().是否找到():
               return "活动界面" if "活动界面" not in self._states else  Field(common_assets.弹框_关闭).查找().点击().随机延时(1, 2)
            if Field(common_assets.队伍弹框界面).设置大图路径(url).查找().是否找到():
               return "队伍弹框界面" if "队伍弹框界面" not in self._states else  Field(common_assets.弹框_关闭).查找().点击().随机延时(1, 2)
            if Field(common_assets.福利弹框界面).设置大图路径(url).查找().是否找到():
               return "福利弹框界面" if "福利弹框界面" not in self._states else  Field(common_assets.福利弹框界面_关闭).查找().点击().随机延时(1, 2)
            if Field(common_assets.便捷组队弹框界面).设置大图路径(url).查找().是否找到():
               return "便捷组队弹框界面" if "便捷组队弹框界面" not in self._states else  Field(common_assets.弹框_关闭).查找().点击().随机延时(1, 2)
            if Field(common_assets.调整组队等级界面).设置大图路径(url).查找().是否找到():
               return "调整组队等级界面" if "调整组队等级界面" not in self._states else  Field(common_assets.弹框_关闭).查找().点击().随机延时(1, 2)
