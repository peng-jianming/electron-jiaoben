import os

通用_跳过 = {
    '标识': '通用_跳过',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'tiaoguo.png')
}

通用_使用 = {
    '标识': '通用_使用',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'shiyong.png')
}

通用_上交 = {
    '标识': '通用_上交',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'shangjiao.png')
}

通用_主界面活动按钮 = {
    '标识': '通用_主界面活动按钮',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'huodong.png')
}

通用_活动界面 = {
    '标识': '通用_活动界面',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'huodongjiemian.png')
}

通用_创建队伍 = {
    '标识': '通用_创建队伍',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'chuangjianduiwu.png')
}

关闭1 = {
    '标识': '通用_关闭1',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'guanbi-1.png')
}

关闭师门弹框 = {
    '标识': '通用_关闭师门弹框',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'guanbi-shimen.png')
}

通用_对话选项框 = {
    '标识': '通用_对话选项框',
    '方式': 'yolo',
    '分类名': '对话框',
    '相似度': 0.7,
    '模型路径': os.path.join(os.path.dirname(__file__), 'best.pt')
}

通用_对话说话框 = {
    '标识': '通用_对话说话框',
    '方式': 'yolo',
    '分类名': '说话框',
    '相似度': 0.7,
    '模型路径': os.path.join(os.path.dirname(__file__), 'best.pt')
}

通用_活动界面未完成参加按钮 = {
    '标识': '通用_活动界面未完成参加按钮',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'canjia.png')
}

通用_弹框_购买 = {
    '标识': '通用_弹框_购买',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'goumai.png')
}

通用_摆摊弹框_购买 = {
    '标识': '通用_摆摊弹框_购买',
    '方式': 'opencv找图',
    '图片路径': os.path.join(os.path.dirname(__file__), 'baitangoumai.png')
}
